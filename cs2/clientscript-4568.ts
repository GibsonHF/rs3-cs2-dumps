//
function script4568(int0: number, int1: number, int2: number, int3: number, int4: number, int5: number, string0: string, string1: string, string2: string): [number, number] {
    ...
    var int8 = 0;
    var int9 = 0;
    var string3 = REMOVETAGS(string1);
    while ((int9 < int1)) {
    ...
    if ((int2 == 1)) {
        ...
        CC_SETONOP(callback(script4569, -2147483644, string3, string2, int4));
        switch (int3) {
            case 9:
            case 20: {
                if ((strcmp(REMOVETAGS(CHAT_PLAYERNAME_UNFILTERED()), REMOVETAGS(string2)) != 0)) {
                    if ((FRIEND_TEST(REMOVETAGS(string2)) == 0)) {
                        if ((IGNORE_TEST(REMOVETAGS(string2)) == 0)) {
