//
function script14419(int0: number, int1: number, int2: number, string0: string): [string, string, string, string, string, string, string, string, string, string] {
    ...
    if ((int0 == 93)) {
        if ((STRING_LENGTH(string1) <= 0)) {
            if ((OC_WEARPOS(int1) != -1)) {
                if ((item_getparam(int1, 3845) == 0)) {
                    if ((enum_getvalue(33, 1, 15589, int1) == 0)) {
                        string1 = OC_IOP(int1, 2);
                    } else if ((((item_getparam(int1, 963) > 0) || (item_getparam(int1, 642) > 0)) || (item_getparam(int1, 964) > 0))) {
                        if (((OC_MEMBERS(int1) == 0) || (varclient_6689 == 1))) {
