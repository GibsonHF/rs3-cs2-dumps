//
function script18167(int0: number): void {
    var string0 = "";
    var string1 = "(Active)";
    var string2 = "(Active)";
    var int1 = 0;
    ...
    if ((int1 == 1)) {
        string0 = `${string0}Double XP time ${string1}: ${script5729((varclient_6727 / 60), MODULO(varclient_6727, 60), varclient_6728, 1, 1)}`;
    };
    if ((int2 == 1)) {
        if ((STRING_LENGTH(string0) > 0)) {
            string0 = `${string0}<br>`;
        };
        string0 = `${string0}Double XP buff time ${string2}: ${script5729((varclient_7339 / 60), MODULO(varclient_7339, 60), varclient_7340, 1, 1)}`;
        string0 = `${string0}<br>Double XP buff days left: ${TOSTRING_LOCALISED((varbitplayer_54478 - DATE_RUNEDAY()), 1)}`;
    };
    script18172(string0, 38273027, -1);
    return;
}