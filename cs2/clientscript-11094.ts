//
function script11094(int0: struct): graphic {
    var int1 = -1 as graphic;
    switch (int0) {
        case 44226: {
            if ((varplayer_1029 >= 0)) {
                int1 = 26027 as graphic;
            };
            break;
        }
        case 44229: {
            if ((varplayer_1031 >= 0)) {
                int1 = 26028 as graphic;
            };
            break;
        }
        case 44231: {
            if ((varplayer_1033 >= 0)) {
                int1 = 26030 as graphic;
            };
            break;
        }
        case 48286: {
            if ((varplayer_11306 >= 0)) {
                int1 = 26030 as graphic;
            };
            break;
        }
        case 44228: {
            if ((varplayer_1035 >= 0)) {
                int1 = 26035 as graphic;
            };
            break;
        }
        case 44227: {
            if ((varplayer_1030 >= 0)) {
                int1 = 26036 as graphic;
            };
            break;
        }
        case 44230: {
            if ((varplayer_1032 >= 0)) {
                int1 = 26038 as graphic;
            };
            break;
        }
        case 44232: {
            if ((varplayer_1034 >= 0)) {
                int1 = 26043 as graphic;
            };
            break;
        }
        case 48287: {
            if ((varplayer_11307 >= 0)) {
                int1 = 26043 as graphic;
            };
            break;
        }
        case 48339: {
            if ((varplayer_11059 != -1 as struct)) {
                int1 = struct_getparam(varplayer_11059, 2802);
            };
            break;
        }
        case 48350: {
            if ((varplayer_11085 == script17464())) {
                int1 = 30127 as graphic;
            };
            break;
        }
        case 52791: {
            if ((varplayer_12655 >= 4)) {
                int1 = 23875 as graphic;
            };
            break;
        }
        case 52064: {
            if ((varplayer_12291 == script20086())) {
                int1 = 35068 as graphic;
            };
            break;
        }
        case 47801: {
            if ((varplayer_10936 == 6)) {
                int1 = 30127 as graphic;
            };
            break;
        }
        case 48878: {
            int1 = script18434(varbitplayer_54950);
            break;
        }
        case 29050: {
            if ((varbitplayer_56961 == 0)) {
                int1 = 33555 as graphic;
            };
            break;
        }
        case 1875: {
            int1 = enum_getvalue(0, 23, 3875 as cs2enum, varbitplayer_3305);
            break;
        }
        case 1876: {
            int1 = enum_getvalue(0, 23, 3875 as cs2enum, varbitplayer_24877);
            break;
        }
        case 1877: {
            int1 = enum_getvalue(0, 23, 3875 as cs2enum, varbitplayer_24878);
            break;
        }
        case 1878: {
            int1 = enum_getvalue(0, 23, 3875 as cs2enum, varbitplayer_24879);
            break;
        }
    };
    return int1;
}