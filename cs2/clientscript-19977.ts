...
function script19977(int0: number, int1: number, int2: number): void {
    script6876(1);
    CC_DELETEALL(comp(1311, 697));  // mtxmgt:hair_recolour_buttons
    var int3 = -1 as dbrow;
    ...
    while ((int10 < int6)) {
        ...
        CC_SETOPCURSOR(1, 46);
        script3537(string0);
        script10033(int10, 2, 0, 0, 1, 1, 4, 4, 1, 1, 4128);
        CC_SETCOLOUR(int7);
        CC_SETFILL(1);
        if ((int10 >= int5)) {
            script10033(int10, 3, 2, 0, 2, 1, 10, 4, 0, 1, 4128);
            CC_SETCOLOUR(int8);
            CC_SETFILL(1);
        };
        if ((script6488(int9) == 0)) {
            script7918(int10, 4, 0, 0, 1, 1, 6, 6, 1, 1, 14794);
        };
        int10 = (int10 + 1);
    };
    return;
