...
function script1232(int0: number): void {
    CC_DELETEALL(int0);
    ...
    while ((int3 < int4)) {
        ...
        int5 = INV_GETOBJ(0, int3);
        if ((INV_GETOBJ(0, int3) != -1)) {
            CC_SETOBJECT(int5, INV_GETNUM(0, int3));
            CC_SETOPBASE(`<col=FF981F>${OC_NAME(INV_GETOBJ(0, int3))}`);
            CC_SETOP(1, "Bank All");
