...
function script12332(int0: number): void {
    ...
    while ((int3 < int8)) {
        ...
        [int2, int3] = script10419(110231555, 110231556, 0, int2, int6, int3, string0, string1, string1, string2, int9, int10, 1, 41495);
        int2 = (int2 + 2);
    };
    ...
    CC_SETCOLOUR(script10495(3));
    int2 = (int2 + (CC_GETHEIGHT() + 0));
    [int2, int3] = script10429(110231557, 110231558, 0, int2, int6, int3, 10874, -1, "Select the number of skills rows you'd like to display at any one time. If this is lower than your current number of pinned rows you will have to remove them manually. Otherwise rows above the limit will be removed the next time a new row is added.", (varbitplayer_30616 - 1), 0);
    return;
}