//
function script12004(int0: number, int1: number, int2: number, int3: number, int4: number, string0: string): void {
    if ((CC_FIND(int0, int3) == 1)) {
        ...
        CC_SETCLICKMASK(0);
        if ((int0 == comp(1890, 38))) {  // prayer_quick_preset_interface:prayer_buttons
            CC_SETOP(1, "Toggle");
            ...
            if ((IF_FIND(comp(1458, 41)) == 1)) {  // toplevel_v2_prayer:customise
                CC_SETHIDE(false);
